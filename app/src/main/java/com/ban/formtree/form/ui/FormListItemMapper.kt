...
fun List<Page>.toListItems(): ImmutableList<FormListItem> {
    val listItems = mutableListOf<FormListItem>()
    forEach { page ->
        listItems += FormListItem.PageTitle(id = page.id, title = page.title)
        page.items.forEach { collectListItems(item = it, depth = 1, listItems = listItems) }
    }
    return listItems.toImmutableList()
}

private fun collectListItems(item: FormItem, depth: Int, listItems: MutableList<FormListItem>) {
    when (item) {
        is FormItem.Section -> {
            listItems += FormListItem.SectionTitle(id = item.id, title = item.title, depth = depth)
            item.items.forEach { collectListItems(item = it, depth = depth + 1, listItems = listItems) }
        }
        ...
        is FormItem.ChoiceQuestion -> {
            listItems += FormListItem.ChoiceItem(
                ...
                options = item.responseSet.responses.map { response ->
                    ChoiceOption(id = response.id, label = response.label, isSelected = false)
                }.toImmutableList(),
                depth = depth,
            )
        }
